...
### 現在できていること
- ユーザー認証（Laravel Sanctum を用いたログイン／ログアウト）
- イベント日の選択と、その日に紐づくシフト一覧の表示
- 新規ユーザー登録（登録後にログインして利用可能）

### まだ実装できていない部分
- 管理者向け画面（シフト／スケジュール調整機能）

### 未実装の理由
- 授業期間内の開発時間が限られており、まずは一般ユーザー向け機能の完成を優先したため

### 実装方針
- 学習で扱った技術スタックである Laravel（API）＋ React（フロントエンド）を採用し、実際に運用可能な Web アプリケーションを目標として実装した。
- 授業内で学んだ内容を復習しつつ、AI ツールも併用して 1 行ごとにコードの意味を確認しながら、認証・API 通信・画面表示などの処理を丁寧に組み立てた。
